...
class MapRenderer {
    ...
    init() {
        ...
        try {
            const mapObj = (this.mapsNamesJson && Array.isArray(this.mapsNamesJson)) ? this.mapsNamesJson.find((obj) => obj.Key && obj.Key.toLowerCase() == MapName.toLowerCase()) : null;
            if (mapObj && mapObj.Name) {
                this.mapname = mapObj.Name.replaceAll(" ", "").replaceAll("_", "").toLowerCase();
