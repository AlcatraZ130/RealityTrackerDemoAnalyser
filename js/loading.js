...
function loadDemo(link, CredsNeeded)
{
	...
	req.onload = () =>
	{
		...
		window.DataBuffer = checkIfGZAndInflate(buffer);
...
function loadDemoFromFileObject(file) {
	...
	reader.onloadend = () => {
		window.DataBuffer = checkIfGZAndInflate(reader.result)
		if (typeof messageArrayObject !== "undefined" && messageArrayObject) {
			messageArrayObject.messages = []
			messageArrayObject.pos = 0
			if (typeof messageArrayObject.updateNewMessages === "function") {
				messageArrayObject.updateNewMessages()
			}
		}
		stage3LoadingFininshed()
...
function stage3LoadingFininshed()
{
	...
	const mapKeyLower = MapName ? MapName.toLowerCase() : "default";
	downloadManager.download(MapsURL + mapKeyLower + ".png", "image", (img) => {
		MapImage = img;
	...
	// Apply default view distance for vision cone based on view_distances.txt
	if (typeof applyMapDefaultViewDistance === 'function') {
		applyMapDefaultViewDistance(MapName);
	}
...
{
	if (typeof analyserPreloader !== "undefined") analyserPreloader.resetResults()
	if (typeof markerDiagnostics !== "undefined") markerDiagnostics.length = 0
...
function stage4LoadingFininshed()
{
	...
	if (typeof updateAnalyserSliderLabels === "function") {
		try { updateAnalyserSliderLabels(); } catch (e) {}
	}
	if (typeof killfeed_ApplyNoLOSMarkers === "function") {
		try { killfeed_ApplyNoLOSMarkers(); } catch (e) {}
	}
