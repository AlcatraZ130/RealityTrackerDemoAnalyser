...
function killfeed_WriteOne(Kill)
{
	...
	let weaponName = Kill.Weapon
	if (Kill.isNoLOSKill || Kill.isSuspicious) {
		row.classList.add("killfeedRow_Suspicious");
		row.title = "No-LOS Kill Alert: Kill occurred without direct Line-of-Sight (outside vision cone or blocked by building wall/terrain)";
	}
...
function killfeed_ApplyNoLOSMarkers()
{
	if (!eventArrays.kills) return
	for (let i = 0; i < eventArrays.kills.events.length; i++) {
		const kill = eventArrays.kills.events[i]
		const row = killfeed_DOMCollection[i]
		if ((!kill.isNoLOSKill && !kill.isSuspicious) || !row) continue
		row.classList.add("killfeedRow_Suspicious")
		row.title = "No-LOS Kill Alert: Kill occurred without direct Line-of-Sight (outside vision cone or blocked by building wall/terrain)"
	}
}
